...
//                    + "AND `tb_box_packing`.`invoice_pengiriman` NOT LIKE '%JT'";
    ...
    private void button_catatan_perendaman_bahan_mentahActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_button_catatan_perendaman_bahan_mentahActionPerformed
        ...
        try {
            ...
            String Query = "SELECT `tanggal_rendam`, `tb_rendam`.`no_laporan_produksi`, `tb_laporan_produksi`.`no_kartu_waleta`, `cheat_no_kartu`, `cheat_rsb`, `tb_bahan_baku_masuk`.`no_registrasi`, `tb_laporan_produksi`.`kode_grade`, `tb_laporan_produksi`.`jumlah_keping`, `tb_laporan_produksi`.`berat_basah`, `lama_waktu_rendam`, `tb_laporan_produksi`.`memo_lp` \n"
                    + "FROM `tb_rendam` "
