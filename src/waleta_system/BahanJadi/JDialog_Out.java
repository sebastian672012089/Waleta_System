...
public class JDialog_Out extends javax.swing.JDialog {
    ...
    public JDialog_Out(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        try {
            initComponents();
            ...
            Logger.getLogger(JDialog_Out.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
...
//                    + "AND `berat` > 0";
    ...
    public boolean CheckDuplicateBoxAsal(String no_box) {
        int i = table_keluar.getRowCount();
        for (int j = 0; j < i; j++) {
            if (no_box.equals(table_keluar.getValueAt(j, 0).toString())) {
                return true;
            }
        }
        return false;
    ...
    public void count() {
        tot_keping = 0;
        tot_gram = 0;
        int x = table_keluar.getRowCount();
        for (int i = 0; i < x; i++) {
            try {
                tot_keping = tot_keping + Integer.valueOf(table_keluar.getValueAt(i, 2).toString());
                tot_gram = tot_gram + Float.valueOf(table_keluar.getValueAt(i, 3).toString());
            } catch (NullPointerException e) {
                JOptionPane.showMessageDialog(this, "Tidak Boleh ada data yang kosong !");
            }
        }
    ...
    private void initComponents() {
        ...
        table_keluar.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "No Box", "Grade", "Kpg", "Gram", "Keterangan"
            }
        ) {
        ...
        table_keluar.getTableHeader().setReorderingAllowed(false);
        jScrollPane2.setViewportView(table_keluar);

        jLabel4.setBackground(new java.awt.Color(255, 255, 255));
        jLabel4.setFont(new java.awt.Font("Arial", 0, 11)); // NOI18N
        jLabel4.setText("Tabel Data Pengeluaran Barang Jadi");
        ...
        jLabel5.setText("Pengeluaran Barang Jadi");
    ...
    private void button_saveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_button_saveActionPerformed
        // TODO add your handling code here:
        int keping_box = 0, gram_box = 0;
        int update_keping, update_gram;
        boolean check = true;
        count();
        try {
            for (int i = 0; i < table_keluar.getRowCount(); i++) {
                if (table_keluar.getValueAt(i, 2) == null || "0".equals(table_keluar.getValueAt(i, 3).toString()) || table_keluar.getValueAt(i, 3) == null) {
                    check = false;
                    throw new Exception("Maaf, Keping tidak boleh kosong, dan gram tidak boleh 0");
                }
            }
        } catch (Exception e) {
        ...
        if (check) {
            try {
                ...
                for (int i = 0; i < table_keluar.getRowCount(); i++) {
                    String data_box = "SELECT `keping`, `berat` FROM `tb_box_bahan_jadi` WHERE `no_box`='" + table_keluar.getValueAt(i, 0) + "'";
                    ResultSet result = Utility.db.getStatement().executeQuery(data_box);
                    if (result.next()) {
                        keping_box = result.getInt("keping");
                        gram_box = result.getInt("berat");
                    }
                    update_keping = keping_box - (int) table_keluar.getValueAt(i, 2);
                    update_gram = gram_box - (int) table_keluar.getValueAt(i, 3);
                    String lokasi_terakhir = null;
                    if (update_keping < 1 && update_gram < 1) {
                        lokasi_terakhir = "OUT";
                    } else {
                        lokasi_terakhir = "GRADING";
                    }

                    String update = "UPDATE `tb_box_bahan_jadi` SET `keping`='" + update_keping + "',`berat`='" + update_gram + "',`status_terakhir`='OUT',`lokasi_terakhir`='" + lokasi_terakhir + "',`tgl_proses_terakhir`='" + dateFormat.format(date) + "' "
                            + "WHERE `no_box` = '" + table_keluar.getValueAt(i, 0) + "'";
                    Utility.db.getConnection().createStatement();
                    Utility.db.getStatement().executeUpdate(update);

                    String out = "INSERT INTO `tb_box_keluar`(`no_box`, `tanggal_keluar`, `keping`, `gram`, `keterangan`) "
                            + "VALUES ('" + table_keluar.getValueAt(i, 0) + "','" + dateFormat.format(date) + "','" + table_keluar.getValueAt(i, 2) + "','" + table_keluar.getValueAt(i, 3) + "','" + table_keluar.getValueAt(i, 4) + "')";
                    Utility.db.getConnection().createStatement();
                    Utility.db.getStatement().executeUpdate(out);
                }
                JOptionPane.showMessageDialog(this, "Data Pengeluaran Box Bahan jadi berhasil di tambahkan");
            ...
            if (Table_daftarBox.getRowCount() == 1) {
                if (CheckDuplicateBoxAsal(Table_daftarBox.getValueAt(0, 0).toString())) {
                    ...
                    model.addRow(new Object[]{Table_daftarBox.getValueAt(0, 0),
                        ...
                        Table_daftarBox.getValueAt(0, 4), null});
        ...
        if (evt.getClickCount() == 2) {
            ...
            if (CheckDuplicateBoxAsal(Table_daftarBox.getValueAt(x, 0).toString())) {
                ...
                model.addRow(new Object[]{Table_daftarBox.getValueAt(x, 0),
                    ...
                    Table_daftarBox.getValueAt(x, 4), null});
                count();
            }
        }
    ...
    private void button_hapus_boxAsalActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_button_hapus_boxAsalActionPerformed
        // TODO add your handling code here:
        int i = table_keluar.getSelectedRow();
        DefaultTableModel model = (DefaultTableModel) table_keluar.getModel();
        if (i != -1) {
            model.removeRow(table_keluar.getRowSorter().convertRowIndexToModel(i));
        }
        count();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(JDialog_Out.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        }
