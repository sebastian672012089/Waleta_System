...
public class JDialog_Select_KaryawanGrup extends javax.swing.JDialog {
    ...
    public JDialog_Select_KaryawanGrup(java.awt.Frame parent, boolean modal, String kode_grup) {
//    public Browse_Karyawan(java.awt.Frame parent, boolean modal) {
        ...
        try {
            ...
            sql = "SELECT `nama_bagian` FROM `tb_bagian` ORDER BY `nama_bagian`";
    ...
    public void refreshTable() {
        try {
            String bagian = " AND `tb_bagian`.`nama_bagian` = '" + ComboBox_bagian.getSelectedItem().toString() + "' ";
            if (ComboBox_bagian.getSelectedItem().toString().equals("All")) {
                bagian = "";
            }
            ...
            sql = "SELECT `tb_karyawan`.`id_pegawai`, `nama_pegawai`, `nama_bagian`, `status`,  `tb_grup_cabut`.`kode_grup`"
                    ...
                    + "WHERE `status` = 'IN' "
                    + "AND `nama_pegawai` LIKE '%" + txt_search_nama_pegawai.getText() + "%' "
                    + "AND `tb_grup_cabut`.`id_pegawai` IS NULL "
                    + bagian
                    + posisi
                    + "GROUP BY `tb_karyawan`.`id_pegawai` ORDER BY `tb_karyawan`.`id_pegawai` DESC";
            rs = Utility.db.getStatement().executeQuery(sql);
    ...
    private void initComponents() {
        ...
        table_list_karyawan.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID Pegawai", "Nama Pegawai", "Bagian", "Status", "Grup"
            }
        ) {
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        ...
                        .addComponent(txt_search_nama_pegawai, javax.swing.GroupLayout.PREFERRED_SIZE, 80, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
    ...
    private void button_selectActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_button_selectActionPerformed
        // TODO add your handling code here:
        int x = table_list_karyawan.getSelectedRow();
        if (x == -1) {
            JOptionPane.showMessageDialog(this, "No Data Selected !");
        } else {
            try {
                int no_urut = 0;
                sql = "SELECT MAX(`kode_urut`) AS 'MAX' FROM `tb_grup_cabut` WHERE `kode_grup` = '" + kode_grup + "' ";
                rs = Utility.db.getStatement().executeQuery(sql);
                if (rs.next()) {
                    no_urut = rs.getInt("MAX") + 1;
                }
                Utility.db.getConnection().setAutoCommit(false);
                String id = table_list_karyawan.getValueAt(x, 0).toString();
                sql = "INSERT INTO `tb_grup_cabut`(`id_pegawai`, `kode_grup`, `kode_urut`) "
                        + "VALUES ('" + id + "','" + kode_grup + "','" + no_urut + "')";
