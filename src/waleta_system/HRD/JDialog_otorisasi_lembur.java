...
public class JDialog_otorisasi_lembur extends javax.swing.JDialog {
        ...
        try {
            ...
            if (jenis.equals("disetujui")) {
                sql = "SELECT * FROM `tb_login` "
                        ...
                        + "WHERE `user`=? AND `pass`=? AND `status` = 'IN' AND (`posisi` = 'MANAGER' OR `posisi` LIKE 'STAFF%')";
            } else if (jenis.equals("diketahui")) {
                sql = "SELECT * FROM `tb_login` "
                        + "JOIN `tb_karyawan` ON `tb_login`.`id_pegawai` = `tb_karyawan`.`id_pegawai` "
                        + "JOIN `tb_bagian` ON `tb_bagian`.`kode_bagian` = `tb_karyawan`.`kode_bagian` "
                        + "WHERE `user`=? AND `pass`=? AND `status` = 'IN' AND `kode_departemen` = 'HRGA' AND (`posisi` = 'MANAGER' OR `posisi` LIKE 'STAFF%')";
            }
            ...
            if (rs.next()) {
                Access = true;
                Pemberi_otorisasi = rs.getString("nama_pegawai");
                JOptionPane.showMessageDialog(this, "Lembur " + jenis + " " + Pemberi_otorisasi);
            } else {
                Access = false;
            }
        } catch (Exception ex) {
            Logger.getLogger(JDialog_otorisasi_lembur.class.getName()).log(Level.SEVERE, null, ex);
        }
        return Access;
    }
    ...
    public void OK() {
        if (getAccess()) {
            try {
                if (jenis.equals("disetujui")) {
                    sql = "UPDATE `tb_surat_lembur` SET `disetujui`='" + Pemberi_otorisasi + "' WHERE `nomor_surat` = '" + SPL + "'";
                    Utility.db.getConnection().createStatement();
                    Utility.db.getStatement().executeUpdate(sql);
                } else if (jenis.equals("diketahui")) {
                    sql = "UPDATE `tb_surat_lembur` SET `diketahui`='" + Pemberi_otorisasi + "' WHERE `nomor_surat` = '" + SPL + "'";
                    Utility.db.getConnection().createStatement();
                    Utility.db.getStatement().executeUpdate(sql);
                }
            ...
            JOptionPane.showMessageDialog(this, "Silahkan menghubungi STAFF / HRD untuk melakukan otorisasi", "Access Denied !", JOptionPane.ERROR_MESSAGE);
        }
        this.dispose();
    }
