 ...
 * <br><br>
 *
 * <li>ACR122U Reader Helper Utility is used to read,write,encrypt and decrypt
 * Mifare Classic 1k Cards only.</li>
 */
public class ACR122U_ReaderHelper {

    /**
     * ACR122UReaderHelper Instance *
     */
    private static ACR122U_ReaderHelper instance;

    /**
     * ACR122UReaderHelper Logger *
     */
    private final ACR122U_Util util = ACR122U_Util.getInstance();
     ...
     */
    private ACR122U_ReaderHelper() {
    }

    public static ACR122U_ReaderHelper getInstance() {
        if (null == instance) {
            instance = new ACR122U_ReaderHelper();
        }
        return instance;
    }
     ...
     * @return boolean
     *
     * <li>This method will return true, If it is connected to ACR122U
     * Reader.</li>
     */
...
//                System.out.println("Common Exception occurred while reading card " + e);
                return false;
            }
     ...
     * @return byte[]
     *
     * This method is used to read the card block using default key which is an
     * unique key to Mifare Classic 1K Cards
     */
     ...
     * @return byte []
     *
     * <li>This method has 3 parameters, user defined authentication key,
     * Authentication command for Key A or Key B, card block to read and return
     * byte Array.</li>
     */
    public byte[] readCardBlock(byte[] authKeyData, byte[] authKeyCmd, int blockNo) {
        if (null == authKeyData || authKeyData.length != 6) {
            throw new ACR122U_CardDataException("Insufficient authKeyData length. Length should be 6 in size");
        }
     ...
     * @return byte[]
     *
     * <li>This method has 4 parameters, user defined authentication key,
     * Authentication command for Key A or Key B, card block to write, byte data
     * of 16 bytes to write in the card.</li>
     */
    public byte[] writeDataIntoCard(byte[] authKeyData, byte[] authKeyCmd, int blockNo, byte[] data) {
        if (null == authKeyData || authKeyData.length != 6) {
            throw new ACR122U_CardDataException("Insufficient authKeyData Param length. Length should be 6 in size");
        }
        if (null == data || data.length != 16) {
            throw new ACR122U_CardDataException("Insufficient data Param length. Length should be 16 in size");
        }
     ...
     * @return byte[]
     *
     * <li>This method is used to encrypt card using default key. Once the card
     * is encrypted the key will be stored on Key B side.</li>
     * <li>We have used access bits to grant read and write access internally to
     * the reader. At the same time if we try to read this block the output will
     * be 0.</li>
     */
    public byte[] encryptCardUsingDefaultKey(byte[] existingAuthKey, byte[] authCmdToAccess, byte[] newAuthKey, int trailerBlockNo) {
        ...
        if (null == existingAuthKey || existingAuthKey.length != 6) {
            throw new ACR122U_CardDataException("Insufficient existingAuthKey Param length. Length should be 6 in size");
        }
        ...
        try {
            ...
            if (tBlock != trailerBlockNo) {
                throw new ACR122U_CardDataException("Invalid Trailor Block ".concat(String.valueOf(trailerBlockNo)));
            }
        } catch (ArrayIndexOutOfBoundsException aiobe) {
//            System.out.println("Error while reading trailerBlock Array " + aiobe);
            throw new ACR122U_CardDataException("Invalid Trailor Block ".concat(String.valueOf(trailerBlockNo)));
        }
...
//		Update Security Key
     ...
     *
     * <li>This method has 3 parameters, user defined authentication key,
     * Authentication command for key A or key B, trailer block number where the
     * authentication key is stored.</li>
     * <li>This method will override the current authentication key with default
     * key(i.e., The key was provided by the manufacturer)</li>
     */
...
//            System.out.println("Error while reading trailerBlock Array " + aiobe);
     ...
     * @return byte Array
     *
     * <li>This method will read card code from manufacturer block and returns
     * the same.</li>
     */
    public byte[] getUID() {
    ...
    private byte[] sendCommand(byte[] apduCmd) {
        if (null == card) {
            throw new ACR122U_CardDataException("Place card on the reader");
        }
...
//            System.out.println("Error occurred while transmitting command to card " + ce);
        }
     ...
     * @return ResponseApdu
     *
     * <li>This method is used to send Apdu command directly and will return
     * ResponseAPDU.</li>
     */
...
//            System.out.println("Error occurred while transmitting command to card " + ce);
     ...
     * @throws CardException
     *
     * <li>This method will make the application wait for card to be present on
     * the reader.</li>
     */
