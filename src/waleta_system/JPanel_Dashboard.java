...
public class JPanel_Dashboard extends javax.swing.JPanel {
...
//                Color a = new Color((int) 255 - (int) valueColor, (int) valueColor, 0);
        ...
        ((BarRenderer) cp4.getRenderer()).setBarPainter(new StandardBarPainter() {
            ...
            public void paintBar(Graphics2D g2, BarRenderer renderer, int row, int column, RectangularShape bar, RectangleEdge base) {
                ...
                float valueColor = 256 * dataset6.getValue(row, column).floatValue() / max;
                if (valueColor % 256 == 0 && valueColor != 0) {
                    valueColor = 256;
                } else {
                    valueColor = valueColor % 256;
                }
                ...
                System.out.println(row + "-" + column + "=" + dataset6.getValue(row, column));
            }
        ...
        try {
            ...
            if (Date_Tandon.getDate() != null) {
                sql = "SELECT "
                        ...
                        + "(SELECT COUNT(tgl_mulai_cetak) FROM `tb_laporan_produksi` LEFT JOIN `tb_cetak` ON `tb_laporan_produksi`.`no_laporan_produksi` = `tb_cetak`.`no_laporan_produksi` WHERE tgl_mulai_cetak='" + dateFormat.format(Date_Tandon.getDate()) + "') AS `cetak_lp`, "
                        ...
                        + "(SELECT COUNT(tgl_masuk_f2) FROM `tb_laporan_produksi` LEFT JOIN `tb_finishing_2` ON `tb_laporan_produksi`.`no_laporan_produksi` = `tb_finishing_2`.`no_laporan_produksi` WHERE tgl_masuk_f2='" + dateFormat.format(Date_Tandon.getDate()) + "') AS `f2_lp`, "
                        ...
                        + "(SELECT COUNT(`tb_lab_laporan_produksi`.`tgl_masuk`) FROM `tb_laporan_produksi` LEFT JOIN `tb_lab_laporan_produksi` ON `tb_laporan_produksi`.`no_laporan_produksi` = `tb_lab_laporan_produksi`.`no_laporan_produksi` WHERE `tb_lab_laporan_produksi`.`tgl_masuk`='" + dateFormat.format(Date_Tandon.getDate()) + "') AS `qc_lp`, "
                        + "(SELECT SUM(berat_basah) FROM `tb_laporan_produksi` LEFT JOIN `tb_lab_laporan_produksi` ON `tb_laporan_produksi`.`no_laporan_produksi` = `tb_lab_laporan_produksi`.`no_laporan_produksi` WHERE `tb_lab_laporan_produksi`.`tgl_masuk`='" + dateFormat.format(Date_Tandon.getDate()) + "') AS `qc_basah`, "
                        + "(SELECT SUM(jumlah_keping) FROM `tb_laporan_produksi` LEFT JOIN `tb_lab_laporan_produksi` ON `tb_laporan_produksi`.`no_laporan_produksi` = `tb_lab_laporan_produksi`.`no_laporan_produksi` WHERE `tb_lab_laporan_produksi`.`tgl_masuk`='" + dateFormat.format(Date_Tandon.getDate()) + "') AS `qc_keping`, "
                        + "(SELECT COUNT(tanggal_grading) FROM `tb_laporan_produksi` LEFT JOIN `tb_bahan_jadi_masuk` ON `tb_laporan_produksi`.`no_laporan_produksi` = `tb_bahan_jadi_masuk`.`kode_asal` WHERE tanggal_grading='" + dateFormat.format(Date_Tandon.getDate()) + "') AS `gbj_lp`, "
                        + "(SELECT SUM(berat_basah) FROM `tb_laporan_produksi` LEFT JOIN `tb_bahan_jadi_masuk` ON `tb_laporan_produksi`.`no_laporan_produksi` = `tb_bahan_jadi_masuk`.`kode_asal` WHERE tanggal_grading='" + dateFormat.format(Date_Tandon.getDate()) + "') AS `gbj_basah`, "
                        + "(SELECT SUM(jumlah_keping) FROM `tb_laporan_produksi` LEFT JOIN `tb_bahan_jadi_masuk` ON `tb_laporan_produksi`.`no_laporan_produksi` = `tb_bahan_jadi_masuk`.`kode_asal` WHERE tanggal_grading='" + dateFormat.format(Date_Tandon.getDate()) + "') AS `gbj_keping` FROM DUAL";
                rs = Utility.db.getStatement().executeQuery(sql);
                dataset6.clear();
                while (rs.next()) {
                    dataset6.setValue(rs.getInt("cuci_basah"), "Berat", "Cuci");
