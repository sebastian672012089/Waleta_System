    ...
    public ArrayList<ProgressLP> prosesList() {
        ...
        try {
            ...
            if (ComboBox_ruangan.getSelectedItem() == "ALL") {
                ruang = "";
            } else if (ComboBox_ruangan.getSelectedItem() == "Eksternal") {
                ruang = "AND LENGTH(`tb_laporan_produksi`.`ruangan`) = 5 \n";
            } else {
            ...
            sql = "SELECT `tb_laporan_produksi`.`no_laporan_produksi`,`tb_laporan_produksi`.`no_kartu_waleta`,`tb_laporan_produksi`.`memo_lp`,`tb_laporan_produksi`.`kode_grade`, `jenis_bulu_lp`, `ruangan`, `jumlah_keping`, `berat_basah`, "
                    ...
                    + "WHERE "
                    + "" + search + " LIKE '%" + txt_search_proses.getText() + "%' "
                    + ruang
                    + "AND YEAR(`tb_laporan_produksi`.`tanggal_lp`)>=2018  "
                    + "AND `berat_basah` > 0 \n"
...
//        label_rata_grading.setText(Double.toString(Math.round(avg_lama_grading)));
    ...
    private void initComponents() {
        ...
        ComboBox_ruangan.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "ALL" }));
