...
public class JDialog_Input_InvoiceHutangExim_PilihInvoice extends javax.swing.JDialog {

    String sql = null;
    ResultSet rs;
    DefaultTableCellRenderer TableAlignment = new DefaultTableCellRenderer();

    public JDialog_Input_InvoiceHutangExim_PilihInvoice(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        table_data.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), "Enter");
        table_data.getActionMap().put("Enter", new AbstractAction() {
            @Override
    ...
    public void refreshTable() {
        try {
            ...
            model.setRowCount(0);
            sql = "SELECT `tb_payment_report`.`invoice`, `tb_buyer`.`nama` "
                    + "FROM `tb_payment_report` "
                    + "LEFT JOIN `tb_hutang_exim` ON `tb_payment_report`.`invoice` = `tb_hutang_exim`.`invoice`\n"
                    + "LEFT JOIN `tb_buyer` ON `tb_payment_report`.`buyer` = `tb_buyer`.`kode_buyer`\n"
                    + "WHERE "
                    + "`tb_hutang_exim`.`invoice` IS NULL AND "
                    + "(`tb_hutang_exim`.`invoice` LIKE '%" + txt_search_keyword.getText() + "%' OR "
                    + "`tb_buyer`.`nama` LIKE '%" + txt_search_keyword.getText() + "%') ";
    ...
    private void txt_search_keywordKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txt_search_keywordKeyPressed
        ...
        if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
            refreshTable();
            if (table_data.getRowCount() == 1) {
                table_data.setRowSelectionInterval(0, 0);
                this.dispose();
            }
        }
