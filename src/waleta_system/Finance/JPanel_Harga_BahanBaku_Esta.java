...
public class JPanel_Harga_BahanBaku_Esta extends javax.swing.JPanel implements InterfacePanel {
    ...
    public void refreshTable_perhitungan() {
        try {
            ...
            model.setRowCount(0);
            sql = "SELECT `no_kartu_waleta`, `tb_grading_bahan_baku`.`kode_grade`, `tb_grade_bahan_baku`.`kategori_esta`, `jumlah_keping`, `total_berat`, `harga_bahanbaku`, `tb_harga_baku_esta`.`harga` AS 'harga_esta' \n"
                    ...
                    + "WHERE `no_kartu_waleta` LIKE '%" + txt_cari_kartu.getText() + "%' AND `tb_harga_baku_esta`.`tanggal` = '" + ComboBox_tanggal.getSelectedItem().toString() + "'";
            rs = Utility.db.getStatement().executeQuery(sql);
            Object[] row = new Object[10];
            while (rs.next()) {
                row[0] = rs.getString("no_kartu_waleta");
                row[1] = rs.getString("kode_grade");
                row[2] = rs.getString("kategori_esta");
                row[3] = rs.getFloat("jumlah_keping");
                row[4] = rs.getFloat("total_berat");
                row[5] = rs.getFloat("harga_bahanbaku");
                double harga_waleta = rs.getDouble("total_berat") * rs.getDouble("harga_bahanbaku");
                row[6] = harga_waleta;
                row[7] = rs.getFloat("harga_esta");
                double harga_esta = rs.getDouble("total_berat") * rs.getDouble("harga_esta");
                row[8] = harga_esta;
                row[9] = harga_esta - harga_waleta;
                model.addRow(row);
    ...
    private void initComponents() {
        ...
        Table_PerhitunganEstaWaleta.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "No Kartu", "Grade Waleta", "Grade Esta", "Kpg", "Berat", "Harga Waleta", "Total (Rp.)", "Harga Esta", "Total (Rp.)", "Esta - Waleta"
            }
        ) {
            Class[] types = new Class [] {
                java.lang.String.class, java.lang.String.class, java.lang.String.class, java.lang.Integer.class, java.lang.Integer.class, java.lang.Double.class, java.lang.Double.class, java.lang.Double.class, java.lang.Double.class, java.lang.Double.class
            };
            boolean[] canEdit = new boolean [] {
